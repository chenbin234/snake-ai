...
def main():
    ...
    if torch.backends.mps.is_available():
        save_dir = "trained_models_QRDQN_cnn_mps"
    else:
        save_dir = "trained_models_QRDQN_cnn"
    os.makedirs(save_dir, exist_ok=True)

    # checkpoint_interval * num_envs = total_steps_per_checkpoint
    checkpoint_interval = 15625
    checkpoint_callback = CheckpointCallback(
        save_freq=checkpoint_interval, save_path=save_dir, name_prefix="QR-DQN_snake")
    ...
    model.save(os.path.join(save_dir, "QR-DQN_snake_final.zip"))
